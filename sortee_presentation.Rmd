...
<div class="notes">
 Despite well-developed data and software sharing capabilities, it is still challenging to reproduce published results. Ecology both applied and basic, and evolutionary biology face unique challenges to achieving reproducibility and transparency due to both the types of studies and the types of data and data collection methods that are prevalent in these disciplines. 
...
What approach can we as scientists utilize to achieve transparency and computational reproducibility? 
How can we modernize our workflows to meet increasing data sharing requirements? 
How do modern researchers navigate the plethora of tools available? 
Why the mismatch between code availability and increase in code sharing policy?

Computational Reproducibility refers to The ability to produce equivalent analytical outcomes from the same data set using the same code and software as the original study (Fidler et al., 2017). With this definition in mind we explored the current state of authors sharing code.
</div>
...
What tools and components should we use in our scientific workflows?  A problem that is addressed in the software engineering domain through Devops that researchers could benefit from.
</div>
...
<div class="notes">
**Examples** from ecology and biology demonstrate that using open source tools to automate a data pipeline can deliver up to date data and forecasting, reduce analysis time and deliver results to stakeholders. The scientific community already has certainly increased the uptake of using such tools however, in order to encourage authors' to share code a conceptual framework is needed to structure the scientific workflow.  
</div>
...
By scoping out the processes needed, identifying stakeholders and how to collaborate and communicate results then directs us to design the discrete stages of the pipeline in our chosen scripting language.
</div>
...
<div class="notes">
The pipeline is where the discrete stages are processed by implementing the components in such a way as to meet the design requirements outlined in the scope. Continuous integration can be setup to meet automation requirements such as weekly data reports and more importantly testing expected functionality of the code, the version control system gives the advantage of rolling back errors or feature testing before merging into the master branch and finally, a container preserves the library and dependencies at runtime. 
</div>
